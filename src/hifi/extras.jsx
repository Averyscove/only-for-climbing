    ...
    <HFPhone dark>
      <div style={{ position: 'absolute', top: 54, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '8px 14px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span onClick={() => window.UC && window.UC.navigate('feed')} style={{ color: '#fff', fontFamily: HF.mono, fontSize: 11, letterSpacing: 1.5, fontWeight: 700, cursor: 'pointer' }}>CANCEL</span>
          <span style={{ fontFamily: HF.display, fontSize: 14, color: '#fff', letterSpacing: -0.2, textTransform: 'uppercase' }}>NEW SEND</span>
          <span onClick={() => window.UC && window.UC.navigate('shareA')} style={{ color: HF.accent, fontFamily: HF.mono, fontSize: 11, letterSpacing: 1.5, fontWeight: 800, cursor: 'pointer' }}>SHARE →</span>
        </div>
  ...
  return (
    <HFPhone dark>
      <div style={{ position: 'absolute', inset: 0 }}>
        <Photo src={HF_IMG.send1} height="100%" dim={0.5} />
        <div style={{ position: 'absolute', inset: 0, padding: 28, display: 'flex', flexDirection: 'column', boxSizing: 'border-box', color: '#fff' }}>
          ...
          <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div onClick={() => window.UC && window.UC.navigate('feed')} style={{ background: HF.accent, color: '#fff', padding: '16px 14px', textAlign: 'center', fontFamily: HF.mono, fontSize: 12, letterSpacing: 1.5, fontWeight: 800, boxShadow: `4px 4px 0 ${HF.dark}`, cursor: 'pointer' }}>CREATE PROFILE →</div>
            <div onClick={() => window.UC && window.UC.navigate('feed')} style={{ border: '1.5px solid rgba(255,255,255,0.4)', padding: '14px', textAlign: 'center', fontFamily: HF.mono, fontSize: 11, letterSpacing: 1.5, fontWeight: 700, color: '#fff', cursor: 'pointer' }}>I ALREADY HAVE AN ACCOUNT</div>
          </div>
