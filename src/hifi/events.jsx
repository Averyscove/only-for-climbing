      ...
      <div style={{ position: 'absolute', top: 54, left: 0, right: 0, bottom: 80, overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch' }}>
        ...
        <div style={{ margin: '0 16px 12px', display: 'flex', border: `1px solid ${HF.rule}` }}>
          {[
            ...
            <div key={s.t} onClick={() => window.UC && window.UC.navigate(s.go)} style={{ flex: 1, padding: '8px 0', textAlign: 'center', fontFamily: HF.mono, fontSize: 11, letterSpacing: 1, fontWeight: 700, background: i === 0 ? HF.ink : 'transparent', color: i === 0 ? '#fff' : HF.ink, borderRight: i < 2 ? `1px solid ${HF.rule}` : 'none', cursor: 'pointer' }}>{s.t}</div>
          ))}
        </div>
        ...
        <div style={{ padding: '0 12px' }}>
          {evts.map((e, i) => (
            <div key={i} onClick={() => window.UC && window.UC.navigate('plan')} style={{ display: 'flex', gap: 12, padding: 4, marginBottom: 14, background: HF.card, border: `1px solid ${HF.ruleSft}`, borderRadius: 3, overflow: 'hidden', cursor: 'pointer' }}>
    ...
    <HFPhone>
      <div style={{ position: 'absolute', top: 54, left: 0, right: 0, bottom: 0, overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch' }}>
        <div style={{ padding: '8px 16px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }} />
          <span onClick={() => window.UC && window.UC.navigate('feed')} style={{ fontFamily: HF.mono, fontSize: 11, color: HF.accent, letterSpacing: 1, fontWeight: 800, cursor: 'pointer' }}>POST →</span>
        </div>
